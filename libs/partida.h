...
/*Gera uma pisição aleartoria em funçao de um n[1,3] sendo as respetivas areas de jogo 
...
/*Gera a posição aleartoria inicial para todos os jogadores dependendo de um n[1,3]*/
...
/***********************************************************FUNÇOES PARA A MOSTRAR OS VALORES DO JOGO************************************************************/

/*Função que serve para imprimir os valores formatados do PLAYER */
...
/*Mostra o cabeçalho dos valores de cada ronda desta maneira com n o numero de ronda:
...
/*Mostra na ecra os valores de toda a partida, Campo de jogo com as posições dos jogadores e os valores de cada jogador
Utilizando as funções anteriores para isso*/
...
/***********************************************************FUNÇOES PARA EXPORTAR OS RESULTADOS DO JOGO************************************************************/

/*Exporta a informação de cada jogador aos dos ficheiros correspondentes*/
...
/*Mostrar ficheiros de texto no programa para ter uma melhor apresentação*/
...
/*Exporta a informação da partida para dois ficheiros um .txt(mostrar em texto plano os resultados) 
...
/***********************************************************FUNÇOES QUE DECORREM O JOGO DA PARTIDA************************************************************/

/*Nesta função sao utilizadas funções do ficheiro valoreslaunch.h (para inserir os valores respetivos a cada lançamento)
e a funçao launch.h (para fazer o lançamento de cada bola)*/
...
/*Abre e reproduz em segundo plano as funções de python que mostram os videos*/
void cesto(){
	FILE *py;
    py = popen("py", "w");
    if (py == NULL) {
...
/***********************************************************FUNÇOES QUE VERIFICAM O ESTADO DA PARTIDA************************************************************/

/*Soma os ontos ao jogador de acordo a ronda e se apresenta o bonus, tudo isto em função da zona*/
...
/* Muda de posição do jogador em função ao modo de jogo, posição atual e se foi ou nao cesto o lançamento 
...
/*Depois de ser executado o lançamento verifica se este foi cesto ou nao, para isto utiliza os valores antes ja calculados
no lançamento (funçoes de launch.h)*/
...
/*Ordena aos jogadores do melhor ao menor em função dos seus pontos*/
...
/*Verifica se existem dois ou mais jogadores na primeira posição, sendo verdade realoca memoria para continuar o jogo
...
int VeRondas(PARTIDA* partida,int *lastronda, int *np,int extras)
{
	...
	if(partida->modoDeJogo==COMPETITIVE && j>1){                                        				/////caso seja modo 2-adicionar rondas e temos varios jogadores com a maior pontuaÃ§ao
...
/*ESTRUTURA para saber guardar a informação do vencedor*/
typedef struct VENCEDOR{
	char nome[30];
	int pontuacao;
}VENCEDOR;

/*Como extra se os jogadores envolvidos tiverem uma pontuação > 0 verifica se bateu um recorde sendo assim guarda a informação
...
/***********************************************************FUNÇAO PRINCIPAL ONDE DECORRE TODAS AS RONDAS************************************************************/
